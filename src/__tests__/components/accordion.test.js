...
describe('<Accordion />', () => {
  ...
  it('opens and closes the <Accordion /> component', () => {
    ...
    const whatIsNetflixBody = container.querySelector('[data-testid="accordion-body"]');
    expect(whatIsNetflixBody.classList.contains('closed')).toBe(true);
    fireEvent.click(queryByText('What is Netflix?'));
    expect(whatIsNetflixBody.classList.contains('open')).toBe(true);
